...
For the nominator $\to$ nominee edge type, we compute homophily directly from the nomination archive, which provides per-nomination geographic fields for both nominators and nominees at 94.7--97.7\% completeness. This allows us to use all 25,844 nomination edges with non-missing geography, rather than restricting to the 8,119 edges with resolved Wikidata QIDs on both endpoints. As a robustness check, we re-estimate nominator $\to$ nominee homophily using birth country from Wikidata for the QID-matched subset; results are reported alongside the full-data estimates in table~S4. Similarly, the vetting body $\to$ nominator homophily analysis uses the nomination archive rather than the 87,492 complete bipartite edges constructed in \texttt{edges.csv}. This is because the bipartite expansion (all active vetting body members $\times$ all nominators who submitted that year) produces a fully connected edge set whose homophily ratio is tautologically near 1.0 (since $O$ and $E$ converge when every source connects to every target). Instead, the analysis pairs each nomination record's nominator geography (professional activity country, from the archive) with the corresponding vetting body's geography (birth country, from Wikidata), yielding $N = 25{,}811$ records with complete geography on both sides. This count is lower than the 25,844 nominator $\to$ nominee records because some prize-years lack vetting body members with resolved birth country. Only analyses that require Wikidata attributes for \emph{both} endpoints (e.g., the QID-subset robustness check) are restricted to the matched subset.
