...
Our analysis conducts approximately 46 statistical tests under formal multiple-comparisons control. These comprise: edge-type decomposition ($\sim$10 tests: 5 edge types $\times$ 2--3 geographic scales with sufficient sample size), prize-specific homophily (5 prizes $\times$ 1 scale = 5 tests), and temporal trends (8 decades $\times$ 2 scales = 16 tests), plus selected robustness checks. An additional 25 prize-by-decade tests (5 prizes $\times$ 5 decades) are treated as exploratory and are \emph{not} included in the $\sim$46 count; readers should interpret these with appropriate caution as they are not subject to multiple-comparisons correction. The primary finding (country-level nominator $\to$ nominee homophily ratio 4.85, $p < 0.001$) remains statistically significant after Bonferroni correction ($\alpha_{\text{adjusted}} = 0.05 / 46 \approx 0.0011$), so its interpretation is robust to multiple-comparisons control. Secondary findings (discipline-specific and temporal patterns) comprise more than half of the 46 tests and should be interpreted with caution, recognizing that some may reflect false discoveries. We chose $N = 10{,}000$ permutations because it yields a minimum reportable $p$-value of $(0 + 1)/(10{,}000 + 1) \approx 0.0001$, providing comfortable clearance above the Bonferroni-adjusted significance threshold ($\alpha_{\text{adjusted}} \approx 0.001$) while keeping the total computational burden tractable. We parallelized all permutation tests across available CPU cores using the \texttt{furrr} package in R. We note that the permutation test does not condition on other covariates such as discipline overlap or institutional ties; a covariate-conditioned null that blocks by prize-year would test whether homophily persists beyond compositional differences across strata.
...
Vetting $\rightarrow$ Nominator & country & 25,811 & 5.5\% & 5.4\% & 1.01 & 0.066 \\
