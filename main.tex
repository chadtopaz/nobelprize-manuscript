...
The present study offers the first comprehensive multilayer network analysis of the Nobel Prize selection process, spanning all five original prize categories from 1901 through the most recent year of publicly available nomination data. We construct a five-layer directed network encompassing governing bodies, vetting bodies, nominators, nominees, and laureates, assembling data from over a dozen sources into a reproducible pipeline. This network structure allows us to decompose geographic homophily by edge type, identifying precisely where in the institutional hierarchy geographic sorting enters the system.

Our primary results are threefold. First, geographic homophily is concentrated almost entirely in the nominator $\to$ nominee edge; the institutional edges connecting governing bodies, vetting bodies, and laureates show negligible homophily. Second, this homophily varies across prizes in a pattern consistent with the internationality of each discipline's knowledge networks, from Literature (highest) to Physics (lowest). Third, the homophily ratio has increased over time even as the nominee pool has diversified, revealing a paradox in which the system has become more globally inclusive in absolute terms while becoming more geographically biased in relative terms.
...
\input{Tables/tab1_layer_composition}
...
\input{Tables/tab2_homophily_by_edge_type}
...
\input{Tables/tab3_temporal_homophily}
...
\subsection{Nomination Flow Asymmetry}

The combination of geographic homophily and asymmetric pool sizes generates predictable imbalances in the flow of recognition across regions. Figure~\ref{fig:flow} shows the return ratio (nominations received / nominations sent) for each UN subregion with at least 100 total nominations.

Northern America is the largest net importer of recognition, with a return ratio of 1.44 (6{,}802 nominations received vs.\ 4{,}714 sent, a surplus of $+$2{,}088). Northern Europe is the largest net exporter, with a return ratio of 0.73 (4{,}974 received vs.\ 6{,}802 sent, a deficit of $-$1{,}860). This is noteworthy because Northern Europe contains the governing and vetting bodies that administer the prize: the region that controls the institutional apparatus of the Nobel system sends far more nominations outward than it receives.

Western Europe, the largest participant by volume (20{,}705 total nominations), is nearly balanced (return ratio 0.96). Eastern Asia is a net exporter (0.73), as are Australia and New Zealand (0.62) and Western Asia (0.47). South America is a modest net importer (1.33).

These flow asymmetries are a downstream consequence of the homophily documented in the preceding sections. When nominators preferentially nominate within their own geography, and when the geographic distribution of nominators differs from that of nominees, the result is systematic net flows from regions that are overrepresented among nominators relative to nominees (Northern Europe) to regions that are overrepresented among nominees relative to nominators (Northern America).
